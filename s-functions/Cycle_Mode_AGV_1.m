%---------------------------------------------------------
function [sys, x0, str, ts] = Cycle_Mode_AGV_1(t, x, u, flag, lns, pnts, AGV_1, Rack_RFID) %The last one is the end of the grinding
...
switch flag
    case 0
        ...
        initdata.PrevRfid = AGV_1.Start;
        initdata.Counter = 2;
        initdata.EnableLineFollowing = false;
        initdata.EnableSteering = false;
        initdata.State = 'flw';
        initdata.SteeringMode = AGV_1.Turn;  %-1 left 0 forward 1 right
        ...
        initdata.g = AGV_1.g;
        initdata.RackNum = AGV_1.RackNum;
        initdata.Target = AGV_1.Target;
        set_param(gcb, 'UserData', initdata);
      
    case 3
        sys = mdlOutputs(t, x, u, lns, pnts, Rack_RFID,AGV_1);
    ...
    function sys = mdlOutputs(t, x, u, lns, pnts, Rack_RFID, AGV_1)
        lsens = u(1);
        csens = u(2);
        rsens = u(3);
        numrfid = u(4);    
        ...
        AGV_1.Start = data.Start;
        AGV_1.AGVState = data.AGVState;
        AGV_1.Target = data.Target;
        
        ind = 1;
        if numrfid == AGV_1.Start % AGV is in start RFID  
            if AGV_1.AGVState == 0
            ...
            a = size(targets);
            if a(1) > 1            
                dist1 = distances(AGV_1.g, AGV_1.Start, targets(1,1));
                data.Target = targets(1,1);
                if a(2) > 1 
                    data.RackNum = targets(1,2);
                end;
                for i = 2:size(targets)
                    dist2 = distances(AGV_1.g, AGV_1.Start, targets(i,1));
                    if dist2 < dist1 
                        dist1 = dist2;
