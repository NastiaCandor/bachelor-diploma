%---------------------------------------------------------
function [sys, x0, str, ts] = Control_system_1_AGV_1(t, x, u, flag, lns, pnts,  AGVs) %The last one is the end of the grinding

switch flag
    case 0
        [sys, x0, str, ts] = mdlInitializeSizes;
        initdata.Path = AGVs(1).Path;
        initdata.NavigationPoints = Navigation_1(pnts, lns);
        initdata.PrevRfid = AGVs(1).Start;
        initdata.Counter = 1;
        initdata.EnableLineFollowing = false;
        initdata.EnableSteering = false;
        initdata.State = 'flw';
        initdata.SteeringMode = AGVs(1).Turn;  %-1 left 0 forward 1 right
        initdata.DetectLeftSensorCount = 0;   
        initdata.DetectRightSensorCount = 0;
        
        initdata.id = 1;
        initdata.Start = AGVs(1).Start;
        initdata.Target = AGVs(1).Target;
                        ...
                        elseif ((AGVs(id).Counter + 1) <= size(AGVs(id).Path, 2)) % If next RFID is not the last
                            if ((AGVs(id).Counter + 1) <= size(AGVs(id).Path, 2)) % If others RFID is not the last
                                if (AGVs(id).Arc(2) == AGVs(i).Arc(2)) % If other AGV is going to our next RFID
                                    ...
                                    SOur = distances(AGVs(id).g, AGVs(1).Arc(1), AGVs(id).Arc(2));
                                    ...
                                    elseif (AGVs(id).Arc(1) == AGVs(i).Path(AGVs(i).Counter + 1))                                    
                                        ...
                                        TOur = TOur + addtime;
                                        if (TOur < TOther)
                                            if (AGVs(id).AGVState < AGVs(id).AGVState)
                                                change = true; % Change the path
                                                break;
                                            end;
                                        end;                                    
