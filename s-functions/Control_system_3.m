%---------------------------------------------------------
function [sys, x0, str, ts] = Control_system_3(t, x, u, flag, lns, pnts) %The last one is the end of the grinding

switch flag
    case 0
        [sys, x0, str, ts] = mdlInitializeSizes;
        initdata.Path = [9    3    4    5];%[1    63    70    69    68    67     2    49    66    72    48    64    34    32    30     9    18];
    ...
    function sys = mdlOutputs(t, x, u, lns, pnts)
        ...
        elseif all(data.State == 'str')
            ...
            if (data.SteeringMode == 1)
                ...
                if (data.DetectLeftSensorCount == 3) 
                    data.State = 'flw';
                    data.EnableLineFollowing = true;
                    data.EnableSteering = false;
            ...
            if (data.SteeringMode == -1)
                ...
                if (data.DetectRightSensorCount == 3) 
                    data.State = 'flw';
                    data.EnableLineFollowing = true;
                    data.EnableSteering = false;
