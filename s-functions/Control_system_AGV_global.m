%---------------------------------------------------------
function [sys, x0, str, ts] = Control_system_AGV_global(t, x, u, flag, lns, pnts, AGVs) %The last one is the end of the grinding
...
switch flag
    ...
    function sys = mdlOutputs(t, x, u, lns, pnts, AGVs)
        ...
        if numrfid > 0 
            data.OnCross = true;
             AGVs(id).Arc(1) = numrfid;
             data.rfid = numrfid;
        elseif (numrfid == 0) && (data.OnCross)
            data.PrevRfid = data.rfid;
            data.PrevRfid = data.rfid;
            data.OnCross = false;
            AGVs(id).SetTime = t;
            AGVs(id).afterturn = t + 1;
        end;
        ...
        if s(2) == 1 
            AGVs(id).Arc(2) = 0;            
        elseif (t > 0.01)&&(numrfid > 0)
            AGVs(id).Arc(2) = AGVs(id).Path(AGVs(id).Counter);            
                  ...
                  s = size(AGVs);
                    if s(2) > 1
                        ...
                        elseif ((AGVs(id).Counter + 1) <= size(AGVs(id).Path, 2)) % If next RFID is not the last
                            if ((AGVs(id).Counter + 1) <= size(AGVs(id).Path, 2)) % If others RFID is not the last
                                if (AGVs(id).Arc(2) == AGVs(i).Arc(2)) % If other AGV is going to our next RFID
                                    ...
                                    elseif ( AGVs(id).Path(data.Counter+1) == AGVs(i).Arc(1) )
                                        a = AGVs(i).Path(AGVs(i).Counter+1);
                                        b = AGVs(i).Arc(2);
                                        addtime = AddTime(AGVs,i,AGVs(i).Arc(2),data.NavigationPoints,AGVs(i).Arc(1));
                      ...
                      if change
                          ...
                          data.Path = shortestpath(g1, numrfid, data.Target); 
        ...
        if all(data.State == 'flw')&&(t > AGVs(id).afterturn)
            ...
            if (detectrfid > 0)
                ...
                if (numrfid ~= 9) 
                    ...
                    s = size(data.Path, 2);
                    if (size(data.Path, 2) ~= data.Counter)
        ...
        AGVs(id).PrevRfid = data.PrevRfid;
